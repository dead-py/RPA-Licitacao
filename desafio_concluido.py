...
class ConsultaLicitacoes:

    def __init__(self, script_dir, url, data_filtro, quantidade_licitacoes):
        self.script_dir = script_dir
        self.url = url
        self.data_filtro = data_filtro
        self.quantidade_licitacoes = quantidade_licitacoes


    # Baixa e descompacta o arquivo zip, este sendo excluído após a ação.
    def baixa_csv_licitacoes(self, script_dir=os.path.dirname(__file__),
    ...
    def remove_temp(self, diretorio=os.path.dirname(__file__) + '\\zip_ref'):
    ...
    def filtra_csv(self, arquivo=os.path.dirname(__file__) + '\\zip_ref\\licitacao.csv',
    ...
    def filtra_item(self, arquivo=os.path.dirname(__file__) + '\\zip_ref\\item.csv',
                    cd_orgao='1',
                    nr_licitacao=1,
                    ano_licitacao=2000,
    ...
    def cria_diretorios(self, diretorio=os.path.dirname(__file__)):
        ...
        for index, row in df_licitacao_temp.iterrows():
            ...
            try:
                ...
                self.filtra_item(cd_orgao=codigo_orgao,
                ...
                self.cria_txt_link(diretorio_licitacao, link_licitacao)


    # Gera o txt com o link da licitação.
    def cria_txt_link(self, path=os.path.dirname(__file__) + '\\LICITACAO_2022', url='www.google.com'):
...
if __name__ == "__main__":
    ...
    consulta = ConsultaLicitacoes(script_dir=os.path.dirname(__file__),
                                   url='http://dados.tce.rs.gov.br/organization/tribunal-de-contas-do-estado-do-rio-grande-do-sul',
                                   data_filtro='2022-05-01',
                                   quantidade_licitacoes=30)
    
    consulta.baixa_csv_licitacoes(consulta.script_dir,
                                   consulta.url)

    consulta.remove_temp()
    consulta.filtra_csv(data_filtro=consulta.data_filtro, quantidade_licitacoes=consulta.quantidade_licitacoes)
    consulta.cria_diretorios()
